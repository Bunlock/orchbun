...
export async function rebuildMemoryUnlocked(journal: RunJournal, projectRoot?: string): Promise<void> {
  ...
  const activeDirectNotes = direct.notes.filter((note) => note.status === "active" && !includedDirectIds.has(note.id));
  const directProjections = new Map(activeDirectNotes.map((note) => [note.id, {
  ...
  const supersededDirectIds = new Set(activeDirectNotes.flatMap((note) => note.supersedes));
  ...
  const provenance = `<!--\nGenerated at: ${new Date().toISOString().replace(/\.\d{3}Z$/, "Z")}\nGenerated by: orchbun memory rebuild\nSource: ${activeDirectNotes.length} active direct notes, ${await approvedManifestCount(journal.memoryRoot)} milestone manifests\nSweep: ${sweep ?? "none"}\n-->\n\n`;
  await Promise.all([
    ...
    writeFile(path.join(journal.memoryRoot, "working", "contracts.md"), `${provenance}# APIs and contracts\n\n${compact?.baseline.contracts.map((item) => `- ${item}`).join("\n") || "No APIs or contracts recorded."}\n`),
