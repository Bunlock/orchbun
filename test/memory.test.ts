...
`);
  ...
  assert.doesNotMatch(projectState, /Documented the follow-up economy decision/);
...
`);
  ...
  assert.match(await readFile(path.join(root, "working", "project-state.md"), "utf8"), /Generated by: orchbun memory refresh/);
  assert.deepEqual(await verifyMemory(journal), {
